...
const VERSION="20260707-v48-barkodlu-gorselsiz";
...
function statusLabel(s){return{review:`🔴 ${"İncelenmesi önerilir"}`,safe:`🟢 ${"Tercih edilebilir"}`,alternatif:`🔵 ${t("alternative")}`,dikkat:"🟠 Dikkat",inceleme:`⚪ ${"İncelenmesi önerilir"}`}[s]||s}
...
function renderStats(){const c=counts();stats.innerHTML=`<button class="stat red" data-stat="review"><small>🔴 ${"İncelenmesi önerilir"}</small><b>${c.review}</b></button><button class="stat safe" data-stat="safe"><small>✅ ${"Tercih edilebilir"}</small><b>${c.safe}</b></button><button class="stat green" data-stat="altli"><small>⭐ ${t("withAlt")}</small><b>${c.altli}</b></button><button class="stat gray" data-stat="inceleme"><small>⚪ ${"İncelenmesi önerilir"}</small><b>${c.inceleme}</b></button>`}
...
function renderFilters(){const arr=[["all",t("all")],["review",`🔴 ${"İncelenmesi önerilir"}`],["safe",`✅ ${"Tercih edilebilir"}`],["altli",`⭐ ${t("withAlt")}`],["inceleme",`⚪ ${"İncelenmesi önerilir"}`],["fav",`❤️ ${t("favorites")}`]];quickFilters.innerHTML=arr.map(([k,l])=>`<button class="chip ${filter===k?'active':''}" data-filter="${k}">${l}</button>`).join("")}
...
function altHtml(x){if(x.status==="safe"&&!x.alternatif)return`<div class="altBox"><span>${"Tercih edilebilir"}</span><b>${t("safeInfo")}</b></div>`;if(!hasAlternative(x))return`<div class="altBox"><span>${t("alternative")}</span><b>-</b></div>`;const tags=String(x.alternatif).split(/[;,•]/).map(v=>v.trim()).filter(Boolean).slice(0,8);return`<div class="altBox"><span>${t("alternative")}</span><div class="tags">${tags.map(v=>`<em>${esc(v)}</em>`).join("")}</div></div>`}
  ...
  <div class="adminForm">
    ...
    <label>Durum
      <select id="adminDurum">
        <option value="review">${esc("İncelenmesi önerilir")}</option>
        <option value="safe">${esc("Tercih edilebilir")}</option>
        <option value="alternatif">${esc(t("alternative"))}</option>
        <option value="dikkat">Dikkat</option>
        <option value="inceleme">${esc("İncelenmesi önerilir")}</option>
      </select>
    </label>
  </div>
...
setTimeout(showLegalNoticeIfNeeded,800);
